...
import { EMAIL_REGEX, IDE_REGEX, PHONE_REGEX } from '@app/helpers/regex';
...
export class ModifyProfileComponent {
  ...
  user: IUser = {} as IUser;
  formModifyUser = this.formBuilder.group({
    ...
    ide_id: new FormControl(this.user.ide_id, Validators.pattern(IDE_REGEX)),
  });
  ...
  constructor(
    ...
    this.apiService.getProfile().subscribe(user => {
      this.user = user;
      this.formModifyUser.patchValue(this.user);
  ...
  onModifyUserSubmit(): void {
    ...
    if (this.formModifyUser.valid) {
      const user: IUserToPost = {
        ...
        first_name: this.formModifyUser.get('first_name')?.value ?? "",
        last_name: this.formModifyUser.get('last_name')?.value ?? "",
